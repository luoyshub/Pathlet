...
def computeOptStrajsAdvanced(
    pth, distPairs, pthOptStrajs, strajCov, c1, c3, m, affectedTrajs
):
    ...
    if c3 == 0:
        ...
        while r <= rmax:
            for trID in affectedTrajs:
                ...
                # key = (pth, trID)
                # if key in distPairs:
                #     strajDists = distPairs[key]
                #     # Your existing logic here
                # else:
                #     print(f"Key {key} not found in distPairs")
                # # debug add end
                key = (pth, trID)
                if key not in distPairs:
                    print(f"Key {key} not found in distPairs")
                    continue
                strajDists = distPairs[(pth, trID)]
                ...
                if straj is not None:
                    summation += (
                        strajCov[straj] - c3 * r * dist
                    )  # summation += cov - c3*r*d.
...
def runGreedy(trajs, distPairs, strajCov, ptStraj, strajPth, trajCov, c1, c2, c3):
    ...
    for key, value in distPairs.items():
        ...
        computeOptStrajsAdvanced(
            pth, distPairs, pthOptStrajs, strajCov, c1, c3, len(ptStraj), affectedTrajs
        )
    ...
    while numUnprocessedPts[0] > 0:
        ...
        if x1[1] <= x2[1]:
            ...
            for trID, pair in pthOptStrajs[pth].items():
                ...
                if (
                    straj is None
                ):  # possible when all strajs of trID in pth's kitty have zero coverage
                    continue
        ...
        for path in affectedPathlets:
            ...
            pthOptCovCost[path] = computeCovCostRatio(
...
if __name__ == "__main__":
    if len(sys.argv) != 5:
        ...
        retVal = runGreedy(
            trajs, distPairs, strajCov, ptStraj, strajPth, trajCov, c1, c2, c3
        )
        print("Finished.")
